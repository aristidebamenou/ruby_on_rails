...
def find_beautiful_languages(languages)
  ...
  # languages.select { |language| puts languages[language] }
  languages.select { |_language, attributes| attributes[:is_beautiful?] == true }
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  languages.each do |language, attributes|
    return attributes[:facts][fact_index] if language == language_name
  end
end
